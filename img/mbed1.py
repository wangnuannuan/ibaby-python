...
class Git(object):
    ...
    print("GIT ignore_file: ",ignore_file)
    ...
    def checkout(rev, clean=False):
        ...
        if not branch:
            print("checkout:",[git_cmd, 'checkout', rev] + (['-f'] if clean else []) + ([] if very_verbose else ['-q']))
            popen([git_cmd, 'checkout', rev] + (['-f'] if clean else []) + ([] if very_verbose else ['-q']))

    def update(rev=None, clean=False, clean_files=False, is_local=False):
        logger.debug("update git rev: %s,clean: %s,clean_files；%s",rev,clean,clean_files)
        if not is_local:
            Git.fetch()
        if clean:
...
class Repo(object):
    ...
    def isrepo(cls, path=None):
        for name, _ in scms.items():
            if os.path.isdir(os.path.join(path, '.'+name)):
                logger.debug('scm :%s',os.path.join(path, '.'+name))
                return True
    ...
    def clone(self, url, path, rev=None, depth=None, protocol=None, offline=False, **kwargs):
        ...
        for scm in scms.values():
            ...
            if cache and not os.path.isdir(path):
                ...
                logger.debug("Found matching cached repository")
                try:
    ...
    def get_cache(self, url, scm):
        ...
        logger.debug("get_cache:cpath: %s",cpath)
...
class Program(object):
    ...
    def __init__(self, path=None, print_warning=False):
        ...
        while cd(path):
            ...
            if os.path.isfile(os.path.join(path, Cfg.file)):
                logger.debug('in %s .mbed exists',path)
                self.path = path
                self.is_cwd = False
                break
    ...
    def get_tools_dir(self):
        ...
        logger.debug("get_tools_dir: %s",paths)
    ...
    def _find_file_paths(self, paths, fl):
        for p in paths:
            ...
            if os.path.isdir(path) and os.path.isfile(os.path.join(path, fl)):
                logger.debug("find_file_paths"+fl)
    ...
    def post_action(self):
        ...
        logger.debug("program post_action mbed_toos_apth: %s",mbed_tools_path )
    ...
    def get_env(self):
        env = os.environ.copy()# 环境变量
        env['PYTHONPATH'] = os.path.abspath(self.path)#设置变量PYTHONPATH
        compilers = ['ARM', 'GCC_ARM', 'IAR', 'ARMC6']
        for c in compilers:
            if self.get_cfg(c+'_PATH'):#获取编译器变量值
    ...
    def get_detected_targets(self):
        ...
        try:
            ...
            oldError = None
            if os.name == 'nt':
                oldError = ctypes.windll.kernel32.SetErrorMode(1) # 警用windows错误 框Disable Windows error box temporarily. note that SEM_FAILCRITICALERRORS = 1
            mbeds = mbed_lstools.create()
            detect_muts_list = mbeds.list_mbeds() #返回每个mbed的详细信息
            if os.name == 'nt':
...
class Cfg(object):
    ...
    # Sets config value
    def set(self, var, val): # 在.mbed 添加 var = val 如果var已存在，则删除
    ...
    def get(self, var, default_val=None):
        ...
        for line in lines:
            logger.debug('Cfg get line:%s', line)
            m = re.match(r'^([\w+-]+)\=(.*)$', line)
            logger.debug('Cfg get m: %s', m)
            if m and m.group(1) == var:
                return m.group(2)
        return default_val
    ...
    def cache(self):
        cache_cfg = self.get('CACHE', 'enabled') # .mbed 中CACHE的值，不存在返回'enabled' cache_repositories默认True
        ...
        cache_dir_cfg = self.get('CACHE_DIR', None) # 返回.mbed 中CACHE_DIR的值
...
def new(name, scm='git', program=False, library=False, mbedlib=False, create_only=False, depth=None, protocol=None, offline=False):
    ...
    logger.debug('mbed new d_path: %s',d_path)
    ...
    if program and library:
        ...
        pp = Program(p_path)
        pd = Program(d_path)
        logger.debug('pp.path: %s',pp.path)
        logger.debug('pd.path: %s ',pd.path)
        if pp.path == pd.path:
    ...
    logger.debug("mbed new d_type: %s",d_type)
    ...
    if d_type == 'program':
        ...
        p.path = cwd_root
        p.set_root()
        logger.debug("program p.get_os_dir(): %s",p.get_os_dir())
        if not create_only and not p.get_os_dir() and not p.get_mbedlib_dir():
            ...
            try:
                with cd(d_path): # 到项目路径，添加mbed-os
...
def import_(url, path=None, ignore=False, depth=None, protocol=None, insecure=False, offline=False, top=True):
    global cwd_root # mbed-os #latest url # path http url  of mbed-os
    ...
    logger.debug("import protocol :%s",protocol)
    logger.debug("import insecure : %s",insecure)
    ...
    with cd(repo.path):
        logger.debug("import deploy :%s",repo.path)
...
def add(url, path=None, ignore=False, depth=None, protocol=None, insecure=False, offline=False, top=True):
    ...
    logger.debug("add url lib :%s",lib.name) # program mbed-os
    print(lib.path) # url of mbed-os
    print(repo.url) # project name
    print(repo.rev) # none
    print(repo.cache) # user .mbed/mbed-cache
    print(lib.fullurl) # mbed-os #latest url
    import_(lib.fullurl, lib.path, ignore=ignore, depth=depth, protocol=protocol, insecure=insecure, offline=offline, top=False)
    print("add relpath",relpath(repo.path, lib.path))
...
def deploy(ignore=False, depth=None, protocol=None, insecure=False, offline=False, top=True):
    ...
    repo = Repo.fromrepo()
    repo.ignores()
    print("deploy repo.lib:   ",repo.libs)
    for lib in repo.libs:
        if os.path.isdir(lib.path):
            if lib.check_repo():
                with cd(lib.path):
                    logger.debug("deploy update:%s",lib.rev)
