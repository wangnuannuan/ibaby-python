						...
						for (Oid in homeStateNew.reported[endpoint]) {
							//for (i in homeStateNew.reported[endpoint][Oid]) {
								if (Oid == "temp") {
									...
									widget.settings = {
										"title": "temperature" ,
										"value": ['datasources["' + ["lan", "state", "reported", endpoint, Oid].join('"]["') + '"]'],
										"include_legend": true,
										"legend": "C"
									};
									row[0][col[0]] += 6;
									...
									widget.settings = {
										"title": "lock" ,
										"value": 'datasources["' + ["lan", "state", "reported", endpoint, Oid].join('"]["') + '"]',
										"callback": 'datasources["' + ["lan", "state", "desired", endpoint, Oid].join('"]["') + '"]',
										"on_text": "ON",
										"off_text": "OFF"
									};
								}else if (Oid == "light") {
				...
				for (endpoint in homeStateNew[key]) {
					if (homeStateNew[key][endpoint] === null) {
						...
						for (Oid in homeStateNew[key][endpoint]) {
								...
								homeState.state[key][endpoint][Oid] = homeStateNew[key][endpoint][Oid];
